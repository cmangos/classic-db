...
CREATE TABLE tmp_creature (SELECT guid, id, map FROM creature WHERE map IN (33,34,36,43,47,48,70,90,109,129,189,209,229,230,249,289,309,329,349,389,409,429,449,450,469,509,529,531,533));
CREATE TABLE tmp_gameobject (SELECT guid, id, map FROM gameobject WHERE map IN (33,34,36,43,47,48,70,90,109,129,189,209,229,230,249,289,309,329,349,389,409,429,449,450,469,529,509,531,533));
...
DELETE FROM creature_movement_template WHERE entry IN (SELECT id FROM tmp_creature WHERE id NOT IN (12999)); -- Exclude NPCs spawned in both instance and open world
DELETE FROM creature_addon WHERE guid IN (SELECT guid FROM tmp_creature);
DELETE FROM creature_template_addon WHERE entry IN (SELECT id FROM tmp_creature WHERE id NOT IN(2267,9699,15475)); -- Exclude NPCs spawned in both instance and open world
